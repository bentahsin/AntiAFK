...
public final class AntiAFKPlugin extends JavaPlugin {
    ...
    private IInputCompatibility inputCompatibility;
    ...
    private void setupCompatibility() {
        this.inputCompatibility = new JavaInputCompatibility(this);

        RegisteredServiceProvider<IInputCompatibility> provider = getServer().getServicesManager().getRegistration(IInputCompatibility.class);
        if (provider != null) {
            this.inputCompatibility = provider.getProvider();
            getLogger().info("Daha gelişmiş bir uyumluluk sağlayıcısı bulundu (muhtemelen AntiAFK-Geyser).");
        } else {
            getLogger().info("Varsayılan uyumluluk sağlayıcısı kullanılıyor. Geyser desteği için AntiAFK-Geyser modülünü yükleyin.");
        }
    }
    ...
    public IInputCompatibility getInputCompatibility() {
